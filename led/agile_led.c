...
*                   RT_NULL:异常
*/
agile_led_t *agile_led_create(GPIO_TypeDef *GPIOx,rt_base_t pin, rt_base_t active_logic, const char *light_mode, int32_t loop_cnt)
{
    if (!is_initialized)
    {
    ...
    HAL_GPIO_WritePin(GPIOA,pin,!active_logic);
    return led;
}
...
int agile_led_stop(agile_led_t *led)
{
    ...
    led->active = 0;
    rt_mutex_release(lock_mtx);
    HAL_GPIO_WritePin(GPIOA,led->pin,!led->active_logic);
    return RT_EOK;
}
...
{
    RT_ASSERT(led);
    HAL_GPIO_WritePin(GPIOA,led->pin,!HAL_GPIO_ReadPin(GPIOA,led->pin));
}
